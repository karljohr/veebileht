  ...
  useEffect(() => {
    ...
    fetch("http://localhost:5000/api/user-boxes", {
      ...
      .then((data) => {
        const loadedBoxes = [];
        if (data.boxh)
          for (let i = 0; i < data.boxh; i++) loadedBoxes.push("rare");
        if (data.boxm)
          for (let i = 0; i < data.boxm; i++) loadedBoxes.push("mystic");
        if (data.boxe)
          for (let i = 0; i < data.boxe; i++) loadedBoxes.push("epic");
        if (data.boxl)
          for (let i = 0; i < data.boxl; i++) loadedBoxes.push("legendary");
